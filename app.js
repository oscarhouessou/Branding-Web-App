...
class BrandingVerification {
    ...
    initializePopups() {
        ...
        this.elements.successPopup.innerHTML = `
            <div class="popup-content">
                ...
                <div class="popup-body">
                    <p>Le traitement est terminé et les résultats sont disponibles dans l'onglet "Résultats".</p>
                </div>
        ...
        //         </div>
        //         <div class="popup-body">
        //             <p>Une erreur de connexion s'est produite lors de la communication avec le serveur. Veuillez vérifier votre connexion internet et réessayer.</p>
        //         </div>
        //         <div class="popup-footer">
        //             <button class="btn btn-primary" onclick="this.closest('.popup-overlay').classList.add('hidden')">
        //                 Fermer
        ...
        if (!this.elements.submitButton.disabled) {
            ...
            try {
                ...
                if (result.plate && result.logo) {
                    ...
                    // Send data to append-to-csv endpoint
                    const csvResponse = await fetch(`${this.apiBaseUrl}/append-to-csv`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                    ...
                    if (!csvResponse.ok) {
                        throw new Error('Erreur réseau lors de l\'enregistrement des données');
                    }
